#Hardcoded McCrary Test for Discrete Running Variables

mccrary_discrete <- function (runvar, cutpoint, bin = NULL, bw = NULL, verbose = FALSE, 
...
###--------------------------------Great McCrary Plots-----------------------------------###
#Example Plot#
#We are going to run the McCrary Test From 270 to 398##


mccrary_example <- NULL; 
for (i in 270:398)
{
  tmp <- mccrary_discrete(dataframe$running_variable, i, bin = 1)
  mccrary_example <- data.frame(rbind(mccrary_example, tmp))
}


mccrary_example$zscores <- mccrary_example$rbind.mccrary_example..tmp.
mccrary_example$rbind.mccrary_example..tmp. <- NULL
mccrary_example$running_variable <- c(270:398)
mccrary_example$sign <- ifelse(mccrary_example$zscores>0, "Positive", "Negative")

mccrary_exampleplot <- ggplot(mccrary_example, aes(x = running_variable, y = zscores, color = sign)) + 
  geom_point() + xlab ("Time") + ylab("McCrary Z-Scores") + ggtitle("McCrary Z-Score Plot") + 
  scale_x_continuous(breaks=c(270, 280, 290, 300, 310, 320, 330, 340, 350, 360, 370, 380, 390, 400), labels = c('4:30', '4:40', '4:50',
                                                                                                                ...
                                                                                                                '6:30', '6:40')) + theme(legend.position = "none")
